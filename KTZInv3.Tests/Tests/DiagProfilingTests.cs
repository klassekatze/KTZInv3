...
namespace KTZInv3.Tests.Tests
{
    ...
    public class DiagProfilingTests
    {
        ...
        public void Profile_QueuePriorityAndDiscovery_ThroughSeam()
        {
            ...
            // real script pays. NOTE: GtsGetBlocks is PER-BLOCK — the real
            // MyGridTerminalSystem.GetBlocksOfType scans m_blocks (a HashSet)
            // linearly: 2.9us on 11 blocks ~= 0.26us/block with the type
            // check + IsAccessibleForProgrammableBlock + delegate per element.
            // Other measured values: GetItems 0.3 (26 stacks, flat),
            // GetItemAt 0.1, TransferItemTo ~2, GetQueue 0.01, CustomData 0.1,
            // LcdWrite 0.2 (30 lines), GetAcceptedItems 0.1.
            ...
            ApiCost.UsPerCall[ApiOp.GtsGetBlocks] = 0.26; // per block! (2.9us / 11)
            ...
            Console.WriteLine("  full inventory run ~62t/1.0s. GTS.GetBlocksOfType is O(blocks) - measured");
            Console.WriteLine("  2.9us on 11 blocks (0.26us/block, HashSet scan + delegate per block); it is");
            Console.WriteLine("  a server-killer ONLY when spammed per-tick. Our script calls GTS only at");
            Console.WriteLine("  boot (ResourceLoader) + manual recalc + docking events - the per-tick hot");
            Console.WriteLine("  path (inventory pipeline, refinery mgr, discovery scans) is GTS-free.");
            Console.WriteLine("  Our walk is 0.037ms warm. No bottleneck.");
            Console.WriteLine("===========================================");
